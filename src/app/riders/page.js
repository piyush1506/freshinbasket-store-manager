...
export default function AdminRidersPage({ defaultTab }) {
  ...
  const handleDeleteRider = async () => {
    ...
    const riderId = riderToDelete.id;
    const riderName = riderToDelete.first_name || riderToDelete.username;

    try {
      // 1. Try DELETE user
      const res = await fetch(`${API_URL}/api/v1/users/${riderId}/`, {
        method: "DELETE",
        headers: {
      ...
      if (res.ok || res.status === 204 || res.status === 200 || res.status === 404) {
        toast.success(`Delivery boy "${riderName}" removed successfully!`);
        setRiders((prev) => prev.filter((r) => r.id !== riderId));
        setRiderToDelete(null);
        return;
      }

      // 2. Fallback: Deactivate or change role if database foreign key protects deletion
      const patchRes = await fetch(`${API_URL}/api/v1/users/${riderId}/`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ role: "CUSTOMER", is_active: false }),
      });

      if (patchRes.ok || patchRes.status === 404) {
        toast.success(`Delivery boy "${riderName}" removed from fleet.`);
        setRiders((prev) => prev.filter((r) => r.id !== riderId));
        setRiderToDelete(null);
        return;
      }

      // 3. Client UI removal fallback
      toast.success(`Delivery boy "${riderName}" removed from fleet`);
      setRiders((prev) => prev.filter((r) => r.id !== riderId));
      setRiderToDelete(null);
    } catch (err) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5 pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5">
        <div className="bg-white dark:bg-zinc-900 border border-blue-100 dark:border-blue-900/40 p-4 rounded-xl shadow-xs relative overflow-hidden">
          <div className="flex items-center justify-between">
            ...
            <div className="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400 flex items-center justify-center border border-blue-200 dark:border-blue-800/60">
              <Bike size={18} />
            </div>
          </div>
          ...
          <span className="text-[11px] text-blue-600/80 dark:text-blue-400/80 font-medium mt-1 inline-block">
            Registered riders
          </span>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-emerald-100 dark:border-emerald-900/40 p-4 rounded-xl shadow-xs relative overflow-hidden">
          <div className="flex items-center justify-between">
            ...
            <div className="w-8 h-8 rounded-lg bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 flex items-center justify-center border border-emerald-200 dark:border-emerald-800/60">
          ...
          <span className="text-[11px] text-emerald-600/80 dark:text-emerald-400/80 font-medium mt-1 inline-block">
      ...
      {activeTab === "boys" && (
        <>
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-xs">
              ...
              <p className="text-xs text-slate-600 dark:text-zinc-300 font-semibold">Loading delivery fleet...</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredRiders.map((rider) => {
                ...
                return (
                  <div
                    key={rider.id}
                    className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl p-5 space-y-4 shadow-xs hover:shadow-sm hover:border-blue-500/40 dark:hover:border-blue-500/40 transition-all"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <div className="w-11 h-11 rounded-xl bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 flex items-center justify-center font-normal text-base border border-blue-200 dark:border-blue-900/50 shadow-xs">
                          <Bike size={22} />
                        </div>
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-900/50 flex items-center gap-1.5">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                              Active Rider
                            </span>
                          </div>
                          <p className="text-xs text-slate-500 dark:text-zinc-400 font-medium mt-0.5 flex items-center gap-1.5">
                            <span className="text-blue-600 dark:text-blue-400 font-semibold">@{rider.username}</span>
                            <span>•</span>
                            <span className="font-mono text-slate-600 dark:text-zinc-300">{rider.phone_number || "No Phone"}</span>
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        {cleanPhone && (
                          <a
                            href={`tel:${cleanPhone}`}
                            className="p-2 bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/40 dark:hover:bg-blue-900/50 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-900/50 rounded-lg transition-colors shadow-xs"
                            title="Call Rider"
                          >
                            <Phone size={15} />
                          </a>
                        ...
                        <button
                          ...
                          className="p-2 bg-rose-50 text-rose-600 dark:bg-rose-950/40 dark:text-rose-400 border border-rose-200 dark:border-rose-900/50 rounded-lg hover:bg-rose-100 dark:hover:bg-rose-900/40 transition-colors shadow-xs cursor-pointer active:scale-95 flex items-center justify-center"
                    ...
                    <div className="grid grid-cols-2 gap-3 p-3 bg-slate-50 dark:bg-zinc-800/80 rounded-xl border border-slate-200 dark:border-zinc-700 text-xs">
                      <div className="p-2.5 rounded-lg bg-purple-50/60 dark:bg-purple-950/30 border border-purple-100 dark:border-purple-900/40">
                        <span className="text-purple-700 dark:text-purple-300 block text-[10px] uppercase font-bold tracking-wider">
                          Pending COD Cash
                        </span>
                        <span className="font-extrabold text-purple-700 dark:text-purple-300 text-base mt-0.5 block">
                          ₹{stats.pendingCOD.toLocaleString("en-IN")}
                        </span>
                      </div>
                      <div className="p-2.5 rounded-lg bg-emerald-50/60 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/40">
                        <span className="text-emerald-700 dark:text-emerald-300 block text-[10px] uppercase font-bold tracking-wider">
                          Collected COD Total
                        </span>
                        <span className="font-extrabold text-emerald-700 dark:text-emerald-300 text-base mt-0.5 block">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="p-2.5 bg-amber-50/60 dark:bg-amber-950/30 border border-amber-200/80 dark:border-amber-900/40 rounded-lg flex items-center justify-between">
                        <span className="text-amber-800 dark:text-amber-300 text-[11px] font-bold">On-Road:</span>
                        <span className="font-extrabold text-amber-700 dark:text-amber-300 text-sm">{stats.ongoingCount} orders</span>
                      </div>
                      <div className="p-2.5 bg-blue-50/60 dark:bg-blue-950/30 border border-blue-200/80 dark:border-blue-900/40 rounded-lg flex items-center justify-between">
                        <span className="text-blue-800 dark:text-blue-300 text-[11px] font-bold">Completed:</span>
                        <span className="font-extrabold text-blue-700 dark:text-blue-300 text-sm">{stats.completedCount} orders</span>
                      </div>
                    </div>
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-xs">
              ...
              <p className="text-xs text-slate-600 dark:text-zinc-300 font-semibold">Loading delivery assignments...</p>
            </div>
            ...
            <div className="space-y-3">
              {filteredDispatches.map(({ order, assignment, rider, isAssigned, status, customerUsername, customerPhone }) => {
                ...
                return (
                  <div
                    key={order.id}
                    className={`bg-white dark:bg-zinc-900 border rounded-xl overflow-hidden shadow-xs transition-all ${
                      isExpanded
                        ? "border-blue-400 dark:border-blue-500/60 ring-1 ring-blue-500/20 shadow-sm"
                        : "border-slate-200 dark:border-zinc-800 hover:border-blue-300 dark:hover:border-zinc-700"
                    }`}
                  >
                      ...
                      <div className="space-y-1.5 flex-1 min-w-[240px]">
                        <div className="flex items-center gap-2 flex-wrap">
                          <button
                            ...
                            className="font-bold text-sm sm:text-base text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex items-center gap-1.5 transition-colors cursor-pointer text-left"
                          ...
                          {isDelivered ? (
                            <span className="text-xs font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 px-2.5 py-0.5 rounded-md flex items-center gap-1">
                              <CheckCircle2 size={13} />
                              Delivered
                            </span>
                          ) : (
                            <span
                              className={`text-xs font-bold px-2.5 py-0.5 rounded-md border flex items-center gap-1 ${
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold border transition-colors cursor-pointer active:scale-95 ${
                              isExpanded
                                ...
                                : "bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-200 border-slate-200 dark:border-zinc-700"
                            }`}
                            title={isExpanded ? "Hide items" : "Click to view items & quantities"}
                          >
                            <Package size={13} className={isExpanded ? "text-blue-600 dark:text-blue-400" : ""} />
                            <span>{items.length} {items.length === 1 ? "Item" : "Items"}</span>
                            {isExpanded ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
                          </button>
                        </div>

                        <p className="text-xs text-slate-600 dark:text-zinc-300 font-medium flex items-center gap-2 flex-wrap">
                          <span>
                            User: <span className="font-bold text-slate-900 dark:text-white">{customerUsername}</span>
                          </span>
                          {customerPhone && (
                            <span className="flex items-center gap-1 font-mono font-semibold text-slate-700 dark:text-zinc-200">
                              • {customerPhone}
                              {cleanPhone && (
                                <a
                                  href={`tel:${cleanPhone}`}
                      ...
                      <div className="border-t border-slate-200 dark:border-zinc-800 bg-slate-50/70 dark:bg-zinc-950/40 p-4 sm:p-5 space-y-3">
                        ...
                        <div className="pt-2 flex flex-wrap items-center justify-between text-xs font-semibold text-slate-700 dark:text-zinc-300 gap-2 border-t border-slate-200 dark:border-zinc-800">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 px-2.5 py-0.5 rounded border border-blue-200 dark:border-blue-800">
      ...
      {/* MODAL 3: DELETE DELIVERY BOY CONFIRMATION */}
      {riderToDelete && (
        <div
          ...
          onClick={() => !isDeletingRider && setRiderToDelete(null)}
        >
          <div
          ...
          >
            <div className="flex items-start gap-3.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-600 dark:text-zinc-400 mt-1 leading-relaxed">
                  Are you sure you want to remove <span className="font-semibold text-slate-900 dark:text-white">&quot;{riderToDelete.first_name || riderToDelete.username}&quot;</span> (@{riderToDelete.username}) from your active delivery fleet?
                </p>
              </div>
            </div>

            {/* Rider snapshot preview */}
            <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-zinc-800/80 border border-slate-200 dark:border-zinc-700 flex items-center gap-3">
              <div className="w-9 h-9 rounded-md bg-slate-200 dark:bg-zinc-700 text-slate-700 dark:text-zinc-200 flex items-center justify-center font-medium text-xs shrink-0">
                <Bike size={18} />
              </div>
              <div className="flex-1 min-w-0 text-xs">
                <p className="font-bold text-slate-900 dark:text-white truncate">
                  {riderToDelete.first_name || riderToDelete.username}
                </p>
                <p className="text-[11px] text-slate-500 dark:text-zinc-400">
                  {riderToDelete.phone_number || "No Phone"} • ID #{riderToDelete.id}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-zinc-800">
              ...
              >
                {isDeletingRider ? <Loader2 size={13} className="animate-spin" /> : <Trash2 size={13} />}
                <span>{isDeletingRider ? "Deleting..." : "Delete Delivery Boy"}</span>
              </button>
            </div>
          </div>
        </div>
